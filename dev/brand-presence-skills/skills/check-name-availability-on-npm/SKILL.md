---
name: check-name-availability-on-npm
description: Check if a package name is available on the npm registry. Use when planning to publish a JavaScript/Node.js package and need to verify the package name.
---
...
|------|------|-----------|------|
| {name} | {YYYY-MM-DD} | YES/NO/OURS | [npmjs.com/package/{name}](https://npmjs.com/package/{name}) |
...
- **Available**: YES, NO, or OURS (if you've already registered it)
- **Link**: URL if taken (leave empty if available)

After checking all variations, update the **Cross-Service Summary** section to reflect npm availability.
