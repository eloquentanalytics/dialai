---
name: check-name-availability-on-substack
description: Check if a subdomain is available on Substack. Use when planning to start a newsletter and need to verify the publication URL availability.
---
...
|------|------|-----------|------|
| {name} | {YYYY-MM-DD} | YES/NO | [{name}.substack.com](https://{name}.substack.com) |
| {variation1} | {YYYY-MM-DD} | YES/NO | |
| {variation2} | {YYYY-MM-DD} | YES/NO | |
```

**Column definitions:**
- **Name**: The variation being checked (subdomain portion)
- **Date**: Date of check (YYYY-MM-DD format)
- **Available**: YES or NO
- **Link**: URL if taken (leave empty if available)

After checking all variations, update the **Cross-Service Summary** section to reflect Substack availability.
