---
name: check-name-availability-on-github
description: Check if a repository or organization/user name is available on GitHub. Use when verifying brand name availability for GitHub presence.
---
...
Record all availability checks in `NAME_VARIATION_AVAILABILITY.md`. Add or update the **Code Hosting** section:
...
|------|------|-----------|------|
| {name} | {YYYY-MM-DD} | YES/NO | [github.com/{name}](https://github.com/{name}) |
...
- **Available**: YES, NO, or special status (e.g., "OURS" if you own it)
- **Link**: URL if taken (leave empty if available)

After checking all variations, update the **Cross-Service Summary** section to reflect GitHub availability.
