...
| `metaJson` | `object` | No | Arbitrary client metadata (e.g., `{ source: "automated-check" }`) |
...
Cost tracking fields enable measuring the economic cost of AI delegation. DIAL tracks these per-specialist to answer: what does it cost to delegate this decision to AI?
...
| `metaJson` | `object` | No | Arbitrary client metadata (e.g., `{ reviewedBy: "jane@example.com" }`) |
...
Runs a machine to rest. Creates a session, registers a built-in proposer, and loops through the decision cycle until the goal state is reached.
