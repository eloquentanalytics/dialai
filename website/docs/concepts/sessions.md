...
A session is "at rest" when it reaches its **goal state**—the rest state defined in the machine definition. In the goal state, no further decision cycles are needed. Note that "goal" does not mean "over"—for cyclical workflows, the initial state and goal state can be the same, and the session may transition out of the goal state when new work arrives.
...
The goal state should represent "at rest" or "stable"—a state where no action is needed:
- Good: `approved`, `completed`, `resolved`, `idle`
- Avoid: `processing`, `in_progress`, `waiting`

For cyclical workflows, the goal state and initial state can be the same (e.g., an agent that starts in `idle`, does work, and returns to `idle`).
