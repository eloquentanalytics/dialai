...
src/dialai/
  __tests__/
    unit/
      ...
      machine-validation.test.ts   # DIAL_011–DIAL_022
      register-proposer.test.ts    # DIAL_021–DIAL_035
    ...
    integration/
      ...
      round-management.test.ts     # DIAL_336–DIAL_340
      cost-tracking.test.ts        # DIAL_341–DIAL_344
    e2e/
...
Vitest runs every test *file* in a separate worker thread by default. With 44 test files,
...
| **44 small files** | More files = more workers saturated. Prevents single-file bottleneck. |
...
|----|-----------|-------------|
| DIAL_001 | creates session with valid machine definition | `createSession` returns a session with a unique `sessionId`, `currentState === initialState`, a generated `roundId`, and a `createdAt` timestamp |
| DIAL_002 | session starts in initial state | `currentState` matches `machine.initialState` |
| DIAL_003 | session has empty history on creation | `history` array is `[]` |
| DIAL_004 | session stores full machine definition | Returned session `.machine` deep-equals the input `MachineDefinition` |
| DIAL_005 | each session gets a unique ID | Two calls to `createSession` produce different `sessionId` values |
| DIAL_006 | each session gets a unique roundId | Two sessions have different `currentRoundId` values |
...
| DIAL_044 | rejects multiple execution modes for voter | Same validation as proposer |
| DIAL_045 | rejects no execution mode for voter | Same validation as proposer |
| DIAL_046 | rejects unknown voter strategyFnName | Error: `Unknown voter strategy` |
| DIAL_047 | rejects duplicate specialistId for voter | Error: `Specialist already exists` |
...
| DIAL_052 | rejects multiple execution modes for arbiter | Error: "Multiple execution modes specified" |
| DIAL_053 | rejects no execution mode for arbiter | Error: "No execution mode specified" |
| DIAL_054 | rejects unknown arbiter strategyFnName | Error: `Unknown arbiter strategy` |
| DIAL_055 | rejects duplicate specialistId for arbiter | Error: `Specialist already exists` |
...
| DIAL_059 | proposal stores sessionId and roundId | Proposal linked to correct session and round |
| DIAL_060 | proposal uses session's currentRoundId when roundId omitted | `effectiveRoundId` defaults to `session.currentRoundId` |
| DIAL_061 | proposal stores specialistId | Correct specialist attribution |
| DIAL_062 | proposal stores isHuman flag from specialist | Reflects specialist's `isHuman` registration |
| DIAL_063 | proposal stores reasoning | Free-text reasoning field preserved |
| DIAL_064 | proposal stores metaJson | Arbitrary metadata preserved |
...
| DIAL_067 | proposal stores createdAt timestamp | `createdAt` is a Date |
...
| DIAL_074 | accepts voteFor "A" | Valid vote choice |
| DIAL_075 | accepts voteFor "B" | Valid vote choice |
| DIAL_076 | accepts voteFor "BOTH" | Valid vote choice |
| DIAL_077 | accepts voteFor "NEITHER" | Valid vote choice |
...
| DIAL_080 | vote uses session's currentRoundId when roundId omitted | Defaults correctly |
| DIAL_081 | vote stores specialistId and isHuman | Correct attribution |
| DIAL_082 | vote stores reasoning and metaJson | Preserved |
...
| DIAL_085 | rejects vote for non-existent session | Error: `Session not found` |
| DIAL_086 | rejects vote from non-registered specialist | Error: `Specialist not found` |
...
| DIAL_093 | selection vote stores sessionId and roundId | Correct linkage |
| DIAL_094 | selection vote uses session's currentRoundId when roundId omitted | Defaults correctly |
| DIAL_095 | selection vote stores specialistId and isHuman | Correct attribution |
| DIAL_096 | selection vote stores reasoning and metaJson | Preserved |
...
| DIAL_099 | rejects selection vote for non-existent session | Error: `Session not found` |
| DIAL_100 | rejects selection vote from non-registered specialist | Error: `Specialist not found` |
| DIAL_101 | rejects selection vote from non-voter specialist | Error: `Specialist X is not a voter` |
...
| DIAL_114 | history entries are ordered chronologically | Multiple transitions produce ordered history |
...
| ID | Test Name | Description |
|----|-----------|-------------|
| DIAL_151 | preferA always returns "A" | Deterministic |
| DIAL_152 | preferB always returns "B" | Deterministic |
| DIAL_153 | both always returns "BOTH" | Deterministic |
| DIAL_154 | neither always returns "NEITHER" | Deterministic |
...
| DIAL_163 | randomVoter (mapped as "random") returns all four choices | A, B, BOTH, NEITHER all appear over many runs |
| DIAL_164 | randomAll (mapped as "randomAll") returns only A or B | Only A and B over many runs |
...
| DIAL_169 | preferHighestAlignment throws with no proposals | Error: "No proposals to select from" |
...
| DIAL_217 | exemplar stores copies of proposals | `proposals` array is a copy (not reference) |
| DIAL_218 | exemplar stores copies of votes | `votes` array is a copy |
| DIAL_219 | exemplar stores copies of selectionVotes | `selectionVotes` array is a copy |
...
| DIAL_229 | evaluateAlignment throws for unknown specialist | Error: `Specialist not found` |
...
| DIAL_236 | evaluateAccuracy throws for unknown specialist | Error: `Specialist not found` |
...
| DIAL_240 | disableSpecialist throws for unknown specialist | Error: `Specialist not found` |
...
| DIAL_269 | sessions map contains created sessions | After `createSession`, appears in `sessions` |
| DIAL_270 | specialists map contains registered specialists | After `registerProposer`, appears in `specialists` |
| DIAL_271 | proposals map contains submitted proposals | After `submitProposal`, appears in `proposals` |
| DIAL_272 | votes map contains submitted votes | After `submitVote`, appears in `votes` |
| DIAL_273 | selectionVotes map contains submitted selection votes | After `submitSelectionVote`, appears in `selectionVotes` |
| DIAL_274 | alignmentRecords map keyed by specialistId:machineName | Key format is `"specialistId:machineName"` |
| DIAL_275 | exemplars map contains created exemplars | After `createExemplar`, appears in `exemplars` |
...
| ID | Test Name | Description |
|----|-----------|-------------|
| DIAL_296 | strategyFn invoked with correct ProposerContext shape | All fields present and correct |
| DIAL_297 | strategyFn invoked with correct VoterContext shape | Includes proposalA, proposalB, history |
| DIAL_298 | selectionStrategyFn invoked with correct SelectionVoterContext | Includes machineName, proposals[], history |
| DIAL_299 | strategyFn return value stored as proposal | Function returns `{ transitionName, toState, reasoning }`, stored correctly |
...
| DIAL_302 | arbiter strategyFn receives ArbiterContext | Includes `sessionId`, `roundId`, `proposals`, `votes`, `threshold`, `history`, `machineName` |
| DIAL_303 | arbiter context includes alignmentScores | `ctx.alignmentScores` populated from alignment records |
| DIAL_304 | arbiter context includes selectionVotes | `ctx.selectionVotes` populated |
...
| ID | Test Name | Description |
|----|-----------|-------------|
| DIAL_312 | valid transition accepted | Proposal for transition listed in current state stored |
| DIAL_313 | invalid direct transition rejected | Direct proposal for non-existent transition errors |
...
| DIAL_316 | toState validated against transition definition | Passing wrong toState throws state mismatch |
...
| DIAL_323 | margin formula: (leader - runner_up) / totalAlignment | Exact calculation verified with known values |
| DIAL_324 | margin with single transition is 1.0 | No runner-up means maximum margin |
| DIAL_325 | margin with equal scores is 0.0 | Perfect tie gives no consensus |
| DIAL_326 | getEffectiveThreshold priority: state > machine > arbiter > 0.5 | Full chain tested |

### 30. Human Primacy Integration

| ID | Test Name | Description |
|----|-----------|-------------|
| DIAL_327 | human proposal adds alignment 1.0 to consensus score | Human contribution always weighted at maximum |
| DIAL_328 | human pairwise vote adds alignment 1.0 | Human vote outweighs low-alignment AI votes |
| DIAL_329 | human selection vote triggers immediate consensus | Bypasses arbiter strategy entirely |
| DIAL_330 | human force bypasses consensus score entirely | Human can force different transition than consensus winner |
| DIAL_331 | human force creates exemplar with full context | Stored for future specialist context |
| DIAL_332 | human force updates alignment for all round participants | Proposers, pairwise voters, and selection voters all updated |
| DIAL_333 | human alignment always 1.0 via getAlignmentScore | Regardless of outcomes |
| DIAL_334 | updateAlignment skips human specialists | No alignment record created |
| DIAL_335 | non-human specialist cannot force transition | Returns guardsPass=false |

### 31. Round and Stale Data Management

| ID | Test Name | Description |
|----|-----------|-------------|
| DIAL_336 | proposals from previous round not counted in new round | After transition, old proposals deleted |
| DIAL_337 | votes from previous round not counted | After transition, old votes deleted |
| DIAL_338 | selection votes from previous round not counted | After transition, old selection votes deleted |
| DIAL_339 | stale roundId detected by arbitration | `submitArbitration` with old `roundId` returns `stale: true` |
| DIAL_340 | roundId mismatch prevents execution | Cannot execute based on stale consensus |

### 32. Cost Tracking Integration

| ID | Test Name | Description |
|----|-----------|-------------|
| DIAL_341 | proposal cost fields stored and retrievable | `costUSD`, `latencyMsec`, `numInputTokens`, `numOutputTokens` roundtrip |
| DIAL_342 | vote cost fields stored and retrievable | Same fields on votes |
| DIAL_343 | selection vote cost fields stored and retrievable | Same fields on selection votes |
| DIAL_344 | arbitration cost fields stored and retrievable | Same fields on arbitration results |

---

## End-to-End Tests

### 33. Full Session Lifecycle via `runSession`
...
### 34. Cold Start — Calibration — Autonomous Consensus (Progressive Collapse)
...
### 35. Proposal Clustering End-to-End
...
### 36. Self-Healing and Re-enablement End-to-End
...
### 37. Trip Line (Alignment Degradation) End-to-End
...
### 40. Complex State Machine Topologies
...
### 41. Semantic Isolation Verification
...
### 42. Edge Cases and Error Recovery
...
### 43. Consensus Math Verification (Exact Calculations)
...
| DIAL_411 | worked example: pairwise BOTH vote on same-transition proposals | Full alignment added (not split) |
| DIAL_412 | worked example: pairwise BOTH vote on different-transition proposals | 0.5 × alignment to each |
| DIAL_413 | worked example: all specialists 0 alignment + human 1.0 | Human alone drives consensus to margin 1.0 |
